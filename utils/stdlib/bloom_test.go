...
func TestBloomFilterExists_Error(t *testing.T) {
	bf := NewBloomFilter(10, 2)
	bf.Bitset.Size = 1

	if bf.Exists("hello") {
		t.Errorf("expected Exists to be false when Bitset.IsSet errors")
	}
}

func TestBloomFilterAdd_SetError(t *testing.T) {
	bf := NewBloomFilter(10, 2)
	bf.Bitset.Size = 1
...
func (e errEncoder) Encode(interface{}) error {
	return e.err
}
...
func TestBloomFilterMerge_SizeMismatch(t *testing.T) {
	bf1 := NewBloomFilter(10, 2)
	bf2 := NewBloomFilter(9, 2)

	if err := bf1.Merge(bf2); err == nil {
		t.Errorf("expected error for size/hash mismatch")
	}
}

func TestBloomFilterMerge_IsSetError(t *testing.T) {
	bf1 := NewBloomFilter(10, 2)
	bf2 := NewBloomFilter(10, 2)
	bf2.Bitset.Size = 1

	if err := bf1.Merge(bf2); err != nil {
		t.Errorf("unexpected error in merge: %v", err)
	}
}

func TestBloomFilterMerge_SetError(t *testing.T) {
	bf1 := NewBloomFilter(10, 2)
	bf2 := NewBloomFilter(10, 2)
	if err := bf2.Bitset.Set(0); err != nil {
		t.Fatalf("set bit: %v", err)
	}
	bf1.Bitset.Size = 0

	if err := bf1.Merge(bf2); err == nil {
		t.Errorf("expected set error")
	}
}
...
func TestBloomFilterSerialize_Error(t *testing.T) {
	old := newGobEncoder
	newGobEncoder = func(w io.Writer) gobEncoder {
		return errEncoder{err: errors.New("encode failed")}
	}
	defer func() {
		newGobEncoder = old
	}()
	...
	bf := NewBloomFilter(10, 2)
	if _, err := bf.Serialize(); err == nil {
		t.Errorf("expected serialization error")
	}
}

func TestBloomFilterDeserialize_Error(t *testing.T) {
	bf := NewBloomFilter(10, 2)
	if err := bf.Deserialize([]byte("not-gob")); err == nil {
		t.Errorf("expected deserialization error")
	}
}
