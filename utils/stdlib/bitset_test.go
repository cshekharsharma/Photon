...
func requireBitSetSet(t *testing.T, bs *BitSet, pos uint64) {
	t.Helper()
	if err := bs.Set(pos); err != nil {
		t.Fatalf("unexpected error while setting bit %d: %v", pos, err)
	}
}

func requireBitSetClear(t *testing.T, bs *BitSet, pos uint64) {
	t.Helper()
	if err := bs.Clear(pos); err != nil {
		t.Fatalf("unexpected error while clearing bit %d: %v", pos, err)
	}
}
...
func TestBitSet_OutOfBounds(t *testing.T) {
	...
	if err := bs.Clear(64); err == nil {
		t.Errorf("expected error for out of bounds clear")
	}

	if _, err := bs.IsSet(64); err == nil {
		t.Errorf("expected error for out of bounds isSet")
	}

	if err := bs.Toggle(64); err == nil {
		t.Errorf("expected error for out of bounds toggle")
	}
}
...
func TestCountSetBits(t *testing.T) {
	bs := NewBitSet(64)
	requireBitSetSet(t, bs, 10)
	requireBitSetSet(t, bs, 20)
	requireBitSetSet(t, bs, 30)
...
func TestAllSetFalse(t *testing.T) {
	bs := NewBitSet(128)
	...
	if bs.AllSet() {
		t.Errorf("expected not all bits to be set")
	}
}
...
func TestBitSetString(t *testing.T) {
	...
	requireBitSetSet(t, bs, 1)
	requireBitSetSet(t, bs, 3)
	requireBitSetSet(t, bs, 5)
...
func TestBitSetGetSize(t *testing.T) {
	bs := NewBitSet(128)
	if size := bs.GetSize(); size != 128 {
		t.Errorf("expected size to be 128, got %d", size)
	}
}
